...
/** Signal protocol state held in memory and snapshotted into [Vault]. */
class DesktopSignalStore(private val state: JSONObject) : SignalProtocolStore {
    ...
    private val preKeys: MutableMap<Int, PreKeyRecord> = state.getJSONArray("one_time_prekeys")
        .records { PreKeyRecord(it.base64UrlDecode()) }
        .associateByTo(mutableMapOf()) { it.id }
    private val signedPreKeys =
        mutableMapOf(1 to SignedPreKeyRecord(state.getString("signed_prekey").base64UrlDecode()))
    private val kyberPreKeys: MutableMap<Int, KyberPreKeyRecord> =
        state.getJSONArray("kyber_one_time_prekeys")
            .records { KyberPreKeyRecord(it.base64UrlDecode()) }
            .associateByTo(mutableMapOf()) { it.id }
    private val kyberSigned =
        KyberPreKeyRecord(state.getString("kyber_signed_prekey").base64UrlDecode())
    ...
    override fun saveIdentity(
        ...
        return if (
            previous == null ||
            previous.base64UrlDecode().contentEquals(identityKey.serialize())
        ) {
    ...
    override fun isTrustedIdentity(
        ...
        val previous =
            remoteIdentities.optString(addressKey(address)).takeIf(String::isNotBlank) ?: return true
        return previous.base64UrlDecode().contentEquals(identityKey.serialize())
    }

    override fun getIdentity(address: SignalProtocolAddress): IdentityKey? =
        remoteIdentities.optString(addressKey(address))
            .takeIf(String::isNotBlank)
            ?.let { IdentityKey(it.base64UrlDecode()) }

    override fun loadPreKey(preKeyId: Int): PreKeyRecord =
        preKeys[preKeyId] ?: throw InvalidKeyIdException("no prekey $preKeyId")
    ...
    override fun loadSession(address: SignalProtocolAddress): SessionRecord? =
        sessions.optString(addressKey(address))
            .takeIf(String::isNotBlank)
            ?.let { SessionRecord(it.base64UrlDecode()) }
    ...
    override fun getSubDeviceSessions(name: String): MutableList<Int> =
        sessions.keySet().asSequence()
            .mapNotNull { key ->
                key.substringBeforeLast('|').takeIf { it == name }
                    ?.let { key.substringAfterLast('|').toInt() }
            }
            .filter { it != 1 }
            .toMutableList()

    override fun storeSession(address: SignalProtocolAddress, record: SessionRecord) {
        sessions.put(addressKey(address), record.serialize().base64Url())
    }
    ...
    override fun loadSignedPreKey(id: Int): SignedPreKeyRecord =
        signedPreKeys[id] ?: throw InvalidKeyIdException("no signed prekey $id")
    ...
    override fun loadKyberPreKeys(): MutableList<KyberPreKeyRecord> =
        (kyberPreKeys.values + kyberSigned).toMutableList()
    ...
    override fun containsKyberPreKey(id: Int): Boolean =
        id == kyberSigned.id || kyberPreKeys.containsKey(id)

    override fun markKyberPreKeyUsed(id: Int, signedPreKeyId: Int, baseKey: ECPublicKey) {
        if (id != kyberSigned.id && kyberPreKeys.remove(id) == null) {
            throw ReusedBaseKeyException("Kyber prekey already used")
        }
    }
    ...
    fun snapshot(): JSONObject {
        ...
        state.put(
            ...
            JSONArray(kyberPreKeys.values.map { it.serialize().base64Url() }),
        )
...
private fun <T> JSONArray.records(factory: (String) -> T): List<T> =
    (0 until length()).map { factory(getString(it)) }
