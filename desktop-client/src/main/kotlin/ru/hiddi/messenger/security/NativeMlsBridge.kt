...
/** JNI boundary for the Rust/OpenMLS core shared by Android and desktop. */
object NativeMlsBridge {
    private val loaded = runCatching {
        ...
        val library = candidates.firstOrNull(Files::isRegularFile)
            ?: error("OpenMLS library not found; build group-mls-core in release mode")
...
data class PendingMlsMembershipOperation(
    ...
    companion object {
        fun decodeAll(encoded: ByteArray): List<PendingMlsMembershipOperation> {
            ...
            return buildList(count) {
                repeat(count) {
                    ...
                    val context = input.bytes(input.nonNegativeInt())
                    val commit = input.bytes(input.nonNegativeInt())
                    val welcomeSize = input.nonNegativeInt()
        ...
        private fun ByteBuffer.nonNegativeInt(): Int {
            require(remaining() >= 4)
...
data class MlsAddMemberResult(
    ...
    companion object {
        fun decode(encoded: ByteArray): MlsAddMemberResult {
            require(encoded.size >= 8 && encoded[0] == 1.toByte()) {
                "Повреждён MLS add-member bundle"
            }
