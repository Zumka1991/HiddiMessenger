...
новой epoch. Тест также восстанавливает группу из `StorageProvider`: ratchet
state не сериализуется Hiddi вручную.
...
до отправки Commit/Welcome. Удаление группы создаёт для каждого участника
надёжный маркер, который ACK-ается только после удаления локального MLS state.
